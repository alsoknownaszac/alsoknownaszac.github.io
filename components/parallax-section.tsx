...
export function ParallaxSection({ children, speed = 0.5, className = "", offset = 0 }: ParallaxSectionProps) {
...
export function ParallaxImage({ src, alt, speed = 0.5, className = "", scale = 1.1 }: ParallaxImageProps) {
  const ref = useRef<HTMLDivElement>(null)
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "end start"]
